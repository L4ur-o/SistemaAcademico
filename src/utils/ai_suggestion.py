...
class AISuggestionEngine:
    ...
    def __init__(self):
        ...
        self.subject_keywords = {
            'informatica': {
                'keywords': ['computador', 'pc', 'windows', 'pasta', 'pastas', 'arquivo', 'arquivos', 'programação', 'python', 'java', 'html', 'css', 'javascript', 'software', 'hardware', 'internet', 'navegador', 'email', 'word', 'excel', 'powerpoint', 'sistema', 'aplicativo', 'programa', 'ferramenta', 'dispositivo', 'criar', 'cria', 'crio'],
    ...
    def _identify_subject_area(self, question: str) -> str:
        ...
        # Normalizar pergunta para busca (remover acentos e plural)
        ...
        for subject, data in self.subject_keywords.items():
            for keyword in data['keywords'] + data['synonyms']:
                ...
                # Verificar palavra exata ou como parte de outra palavra
                if keyword_normalized in question_normalized:
                    return subject
                # Verificar plural/singular (básico)
                if keyword_normalized.endswith('a') and keyword_normalized + 's' in question_normalized:
                    return subject
                if keyword_normalized + 's' in question_normalized:
                    return subject
                if keyword_normalized.endswith('s') and keyword_normalized[:-1] in question_normalized:
                    return subject
        return 'geral'
    ...
    def _extract_keywords(self, question: str) -> List[str]:
        ...
        common_words = {'como', 'o', 'que', 'é', 'são', 'para', 'com', 'em', 'de', 'da', 'do', 'das', 'dos', 'um', 'uma', 'uns', 'umas', 'me', 'minha', 'meu', 'nossa', 'nosso', 'quero', 'gostaria', 'preciso', 'desejo', 'fazer', 'aprender', 'saber', 'entender', 'crio', 'criar', 'cria'}
        
        # Dividir em palavras e filtrar
        words = re.findall(r'\b\w+\b', question)
        keywords = [word for word in words if len(word) > 2 and word not in common_words]
        
        # Se não encontrou keywords suficientes, incluir palavras maiores mesmo que sejam comuns
        if len(keywords) < 2:
            # Incluir verbos de ação que podem ser úteis
            action_words = [word for word in words if len(word) > 3 and word not in {'como', 'para', 'com', 'para'}]
    ...
    def suggest_lessons(self, question: str, max_results: int = 3) -> List[Dict]:
        """Sugere aulas baseadas na pergunta do aluno usando RAG (busca semântica)"""
        try:
            # Analisar a pergunta
            ...
            # NÃO filtrar por área - sempre buscar, mesmo que seja "geral"
            # Isso permite encontrar aulas mesmo quando a área não é identificada corretamente
            ...
            # ESTRATÉGIA PRINCIPAL: Busca semântica usando RAG (embeddings)
            suggestions = []
            try:
                from ..utils.embeddings import search_similar_aulas
                # Tentar busca semântica primeiro (RAG)
                semantic_results = search_similar_aulas(question, top_k=max_results * 2)
                if semantic_results:
                    logger.info(f"Busca semântica encontrou {len(semantic_results)} resultados")
                    # Converter score de similaridade para formato esperado
                    for result in semantic_results:
                        # Normalizar score de 0-1 para 0-10
                        similarity_score = result.get('score', 0)
                        result['relevance_score'] = similarity_score * 10 if similarity_score > 0 else 0
                        result['ai_score'] = similarity_score * 10
                    suggestions.extend(semantic_results)
            except Exception as e:
                logger.warning(f"Busca semântica falhou, usando fallback: {e}")
            
            # FALLBACK: Se busca semântica não retornou resultados suficientes, usar palavras-chave
            if len(suggestions) < max_results:
                logger.info("Complementando com busca por palavras-chave")
                
                # Estratégia 1: Busca por palavras-chave principais (mais permissivo)
                if analysis['keywords']:
                    for keyword in analysis['keywords'][:3]:  # Aumentar para 3 palavras
                        results = self.db_manager.search_aulas_keyword(keyword, limit=2)  # Aumentar limite
                        # Reduzir threshold mínimo para 1 (mais permissivo)
                        filtered_results = [r for r in results if r.get('relevance_score', 0) >= 1]
                        suggestions.extend(filtered_results)
                
                # Estratégia 2: Busca pela pergunta completa (sempre tentar)
                full_results = self.db_manager.search_aulas_keyword(question, limit=3)
                # Reduzir threshold para 1
                filtered_full_results = [r for r in full_results if r.get('relevance_score', 0) >= 1]
                suggestions.extend(filtered_full_results)
                
                # Estratégia 3: Busca por área de conhecimento (se identificada)
                if identified_area == 'informatica':
                    subject_keywords = self.subject_keywords.get(identified_area, {}).get('keywords', [])
                    for keyword in subject_keywords[:2]:  # Aumentar para 2 palavras
            ...
            # Se temos resultados de busca semântica, priorizar eles
            if any(s.get('score') for s in unique_suggestions):
                # Ordenar por score de similaridade (maior primeiro)
                unique_suggestions.sort(key=lambda x: x.get('score', -1), reverse=True)
            else:
                # Usar ranking tradicional
                ranked_suggestions = self._rank_suggestions(unique_suggestions, analysis)
                unique_suggestions = ranked_suggestions
            
            # Filtrar por relevância mínima (muito permissivo para garantir resultados)
            # Se temos resultados de busca semântica, aceitar qualquer score > 0
            # Se não, aceitar score >= 1 (muito baixo para não perder resultados válidos)
            if any(s.get('score') for s in unique_suggestions):
                min_score = 0.1  # Muito baixo para busca semântica
            else:
                min_score = 1  # Baixo para busca por palavras-chave
            
            final_suggestions = [s for s in unique_suggestions if s.get('ai_score', s.get('relevance_score', 0)) >= min_score]
            
            # Se ainda não temos resultados, retornar os top N mesmo com score baixo
            if not final_suggestions and unique_suggestions:
                logger.warning("Nenhum resultado passou no filtro de relevância, retornando top resultados")
                final_suggestions = unique_suggestions[:max_results]
            ...
            for suggestion in final_suggestions[:max_results]:
                ...
                # Adicionar flag indicando que foi encontrado via RAG
                if suggestion.get('score'):
                    suggestion['found_via_rag'] = True
                    suggestion['semantic_similarity'] = suggestion.get('score', 0)
            
            logger.info(f"Sugestões encontradas: {len(final_suggestions[:max_results])} (RAG: {sum(1 for s in final_suggestions[:max_results] if s.get('found_via_rag'))})")
