...
class SimpleConnectionPool:
    """Pool simples de conexões para MySQL"""
    def __init__(self, host, port, database, user, password, max_connections=10):
        self.host = host
        self.port = port
        ...
        try:
            ...
            with self.lock:
                if self.created_connections < self.max_connections:
                    ...
                    # Aguardar uma conexão disponível
...
class DatabaseManager:
    ...
    def ensure_aulas_embedding_column(self) -> None:
        ...
        try:
            with self.get_cursor() as cursor:
                # Verificar se a coluna já existe (MySQL não suporta IF NOT EXISTS em ALTER TABLE)
                cursor.execute("""
                    SELECT EXISTS (
                ...
                if not column_exists:
                    ...
                    """)
                    cursor.connection.commit()
                    logger.info("Coluna embedding_json adicionada à tabela aulas")
        except Exception as e:
            logger.warning(f"Erro ao verificar/criar coluna embedding_json: {e}")
    ...
    def search_aulas_by_tag(self, tag: str) -> List[Dict]:
        """Busca aulas por tag"""
        query = "SELECT * FROM aulas WHERE tags LIKE %s ORDER BY titulo"
        params = (f"%{tag}%",)
        return self.execute_query(query, params)

    def search_aulas_keyword(self, query_text: str, limit: int = 5) -> List[Dict]:
        """Fallback melhorado por palavras-chave quando embeddings não estão disponíveis (MySQL compatível)."""
        ...
        # Construir condições OR para cada palavra (MySQL usa LOWER() para case-insensitive)
        conditions = []
        params = []
        
        for word in words:
            like_pattern = f"%{word}%"
            # MySQL: usar LOWER() para busca case-insensitive
            conditions.append("(LOWER(titulo) LIKE LOWER(%s) OR LOWER(descricao) LIKE LOWER(%s) OR LOWER(tags) LIKE LOWER(%s) OR LOWER(legendas) LIKE LOWER(%s))")
        ...
        # Construir query compatível com MySQL (sem ANY(ARRAY[...]))
        # Usar múltiplos OR para simular ANY
        score_conditions = []
        score_params = []
        
        # Score base por campo - título
        for word in words:
            like_pattern = f"%{word}%"
            score_conditions.append("(CASE WHEN LOWER(titulo) LIKE LOWER(%s) THEN 10 ELSE 0 END)")
            score_params.append(like_pattern)
        
        # Score base por campo - descrição
        for word in words:
            like_pattern = f"%{word}%"
            score_conditions.append("(CASE WHEN LOWER(descricao) LIKE LOWER(%s) THEN 3 ELSE 0 END)")
            score_params.append(like_pattern)
        
        # Score base por campo - tags
        for word in words:
            like_pattern = f"%{word}%"
            score_conditions.append("(CASE WHEN LOWER(tags) LIKE LOWER(%s) THEN 5 ELSE 0 END)")
            score_params.append(like_pattern)
        
        # Score base por campo - legendas
        for word in words:
            like_pattern = f"%{word}%"
            score_conditions.append("(CASE WHEN LOWER(legendas) LIKE LOWER(%s) THEN 2 ELSE 0 END)")
            score_params.append(like_pattern)
        
        # Bonus para palavras específicas no título
        for word in specific_words:
            like_pattern = f"%{word}%"
            score_conditions.append("(CASE WHEN LOWER(titulo) LIKE LOWER(%s) THEN 5 ELSE 0 END)")
            score_params.append(like_pattern)
        
        # Bonus extra para palavras muito específicas no título
        ...
        query = f"""
            SELECT *, 
                   ({'+'.join(score_conditions) if score_conditions else '0'}) as relevance_score
            FROM aulas 
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(f"Erro operacional ao testar conexão: {e}")
            if "server closed the connection" in error_msg or "Lost connection" in error_msg:
                logger.error("O servidor do banco de dados fechou a conexão. Verifique se o serviço está ativo.")
            elif "could not connect" in error_msg.lower() or "Can't connect" in error_msg:
                logger.error("Não foi possível conectar ao servidor. Verifique host, porta e credenciais.")
            return False
    ...
    def search_instituicoes(self, search_term: str) -> List[Dict]:
        ...
        query = """
            ...
            WHERE LOWER(nome) LIKE LOWER(%s) OR LOWER(cnpj) LIKE LOWER(%s) OR LOWER(cidade) LIKE LOWER(%s)
            ORDER BY nome
        """
